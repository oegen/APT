<%@ Page Language="VB" AutoEventWireup="false" CodeFile="DocumentHistory.aspx.vb" Inherits="Project_DocumentHistory"
MasterPageFile="~/MasterPages/MasterPage.master" Title="Document History" %>

<%@ Register Src="~/Controls/Layout/GenericTitle.ascx" TagName="GenericTitle" TagPrefix="oegen" %>
<%@ Register Src="~/Controls/Navigation/SubNavProject.ascx" TagName="SubNavProject" TagPrefix="oegen" %>
<%@ Register Src="~/Controls/Layout/ContentHeader.ascx" TagName="ContentHeader" TagPrefix="oegen" %>
<%@ Register Src="~/Controls/Layout/ProjectHeader.ascx" TagName="ProjectHeader" TagPrefix="oegen" %>
<%@ Register Src="~/Controls/Project/DocumentHistoryListing.ascx" TagName="DocumentHistoryListing" TagPrefix="oegen" %>

<asp:Content ID="content1" runat="server" ContentPlaceHolderID="ContentPlaceHolder1">

<div id="maincontent" class="group">
    <!--SECONDARY NAV STARTS-->
        <oegen:SubNavProject ID="ctrlSubNavProject" runat="server" SelectedItem="EDIT_PROJECT" />
    <!--SECONDARY NAV ENDS-->
    <!--CONTENT STARTS-->
    <div id="content" class="group">
                    
        <oegen:ProjectHeader ID="ctrlProjectHeader" runat="server" />
        <!--PROJECT LIST STARTS-->            
            <hr />
            <div id="gen-inner">
                <oegen:GenericTitle ID="ctrlGenericTitle" runat="server" TipTitle="Document History" Description="View all the document history" />
                 
                <!--  GENERIC FORM -->
                                                 
                <div class="aptform gen-form">
                    <oegen:DocumentHistoryListing ID="ctrlDocumentHistoryListing" runat="server" />
                </div> 
            </div>                        
            <!--  END GENERIC FORM -->               
        <!--PROJECT LIST ENDS-->               
    </div>
    <!--CONTENT ENDS-->
                    
</div>

</asp:Content>